...
#include <utility>
Entity::Entity(std::string aName, Entity *aParent, Mesh *aMesh, glm::mat4 aLocalTransform, VkPipelineLayout aPipelineLayout)
    : mName(std::move(aName)), mParent(aParent), mMesh(aMesh), mHasMesh(true), mPipelineLayout(aPipelineLayout) {
    // convert the transformation matrix to a translation, rotation (quaternion), scale
...
void Entity::RecordDraw(VkCommandBuffer aCmdBuff) {
    if(mHasMesh) {
        // push the model matrix
        glm::mat4 mModelMatrix = getWorldTransform();
        vkCmdPushConstants(aCmdBuff, mPipelineLayout,
        ...
        for(auto meshPrimitive : mMesh->meshPrimitives)
        {
            ...
            VkBuffer buffers[] = {meshPrimitive.meshGPU->mPositions.buffer, meshPrimitive.meshGPU->mTexcoords.buffer, meshPrimitive.meshGPU->mNormals.buffer};
            ...
            vkCmdBindIndexBuffer(aCmdBuff, meshPrimitive.meshGPU->mIndices.buffer, 0, VK_INDEX_TYPE_UINT32);
