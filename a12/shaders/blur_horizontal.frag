...
// bloom texture
layout (set = 1, binding = 0) uniform sampler2D uTextureBloom;
...
vec4 getColour(float x, float y)
{
    ...
    vec4 col =  texture(uTextureBloom, vec2(x, y) + v2fTexCoord);
    return col;
}

void main()
{
    vec4 texColour = texture(uTextureColour, v2fTexCoord);
    ...
    texColour = texColour + blurredOutput;
    // apply tonemapping
    texColour = 1.0 - exp(-texColour);
    oColour = texColour;
}