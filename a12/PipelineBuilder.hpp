...
#define VK_CHECK(call, message)                                               \
    do {                                                                      \
        ...
        if (result != VK_SUCCESS) {                                           \
            std::cout << "[VK ERROR]: " << message << " VkResult: " << result \
                      << std::endl;                                           \
        }                                                                     \
    } while (0)
...
enum class ShaderType { VERTEX, FRAGMENT, GEOM, COMPUTE };

enum class VertexBinding { BIND, NONE };
...
namespace vkEngine {
    ...
    class PipelineBuilder {
       public:
        PipelineBuilder(VkDevice device, PipelineType type, VertexBinding binding,
                        uint32_t subpass)
            : device(device),
              subpass{subpass},
              pipelineType(type),
        ...
        PipelineBuilder& AddShader(const std::string& shaderPath, ShaderType type) {
            shaders.push_back(
                {ShaderTypeToVkShaderStage(type), CreateShaderModule(shaderPath)});
            return *this;
        }
        ...
        PipelineBuilder& SetDynamicState(
            ...
            m_dynamicStateInfo = {};
            m_dynamicStateInfo.sType =
                VK_STRUCTURE_TYPE_PIPELINE_DYNAMIC_STATE_CREATE_INFO;
            m_dynamicStateInfo.dynamicStateCount =
        ...
        PipelineBuilder& SetRasterizationState(
            ...
            VkFrontFace frontFace, VkBool32 depthBiasEnable = VK_FALSE,
        ...
        PipelineBuilder& SetPipelineLayout(
            ...
            const std::optional<VkPushConstantRange>& pushConstant = std::nullopt) {
            descriptorLayouts = layouts;

            m_pipelineLayout = {};
            m_pipelineLayout.sType = VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO;
            m_pipelineLayout.setLayoutCount =
        ...
        PipelineBuilder& SetDepthState(VkBool32 depthTestEnable,
       ...
       private:
        ...
        VkPipeline CreateGraphicsPipeline() {
            ...
            if (binding == VertexBinding::BIND) {
                vertexInputInfo.vertexBindingDescriptionCount = static_cast<uint32_t>(3);
            ...
            VK_CHECK(vkCreatePipelineLayout(device, &m_pipelineLayout, nullptr,
                                            &pipelineLayout),
        ...
        VkPipeline CreateComputePipeline() {
            // Compute pipeline requires only a compute shader which should be the
            // first shader provided
