...
vk::ForwardPass::ForwardPass(Context &context, Image &shadowMap,
                             Image &depthPrepass, std::shared_ptr<Scene> &scene,
                             std::shared_ptr<Camera> &camera)
    : context{context}, shadowMap{shadowMap}, depthPrepass{depthPrepass},
      scene{scene}, camera{camera} {
    m_RenderTarget = CreateImageTexture2D(
        "ForwardPassRT", context, context.extent.width, context.extent.height,
        context.swapchainFormat,
        VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT,
        VK_IMAGE_ASPECT_COLOR_BIT, 1);

    m_DepthTarget =
        CreateImageTexture2D("ForwardPassDepth", context, context.extent.width,
                             context.extent.height, VK_FORMAT_D32_SFLOAT,
                             VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT |
                                 VK_IMAGE_USAGE_SAMPLED_BIT,
                             VK_IMAGE_ASPECT_DEPTH_BIT, 1);

    m_BrightnessTexture = CreateImageTexture2D(
        "BrightnessRT", context, context.extent.width, context.extent.height,
...
vk::ForwardPass::~ForwardPass() {
    ...
    vkDestroyPipelineLayout(context.device, m_alphaMaskPipeline.second,
                            nullptr);
    ...
    if (meshDescriptorSetLayout != VK_NULL_HANDLE) {
        vkDestroyDescriptorSetLayout(context.device, meshDescriptorSetLayout,
                                     nullptr);
    }
        if (skinDescriptorSetLayout != VK_NULL_HANDLE) {
                vkDestroyDescriptorSetLayout(context.device, skinDescriptorSetLayout,
                                         nullptr);
        }
}
...
void vk::ForwardPass::Resize() {
    ...
    m_RenderTarget = CreateImageTexture2D(
        "ForwardPassRT", context, width, height, context.swapchainFormat,
        VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT,
        VK_IMAGE_ASPECT_COLOR_BIT, 1);

    m_DepthTarget = CreateImageTexture2D(
        "ForwardPassDepth", context, width, height, VK_FORMAT_D32_SFLOAT,
        VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT |
            VK_IMAGE_USAGE_SAMPLED_BIT,
        VK_IMAGE_ASPECT_DEPTH_BIT, 1);
    ...
    for (size_t i = 0; i < (size_t)MAX_FRAMES_IN_FLIGHT; i++) {
        ...
        UpdateDescriptorSet(context, 2, imageInfo, m_descriptorSets[i],
...
void vk::ForwardPass::CreateRenderPass() {
    ...
    m_renderPass =
        builder
            .AddAttachment(context.swapchainFormat, VK_SAMPLE_COUNT_1_BIT,
            ...
            .SetDepthAttachmentRef(
                0, 2, VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL)
            .AddColorAttachmentRef(0, 0,
                                   VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL)
            .AddColorAttachmentRef(0, 1,
                                   VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL)
            // External -> 0 : Color
            ...
            // External -> 0 : Depth
            // Wait for the depth-prepass to finish writing to the depth
            // attachment before this pass uses it for depth comparison
            //.AddDependency(
            ...
            // 0 -> External : Depth
            // Wait for this pass to finish reading from the depth attachment to
            // occlude fragments before the depth-prepass writes to it
            //.AddDependency(0, VK_SUBPASS_EXTERNAL,
            //	VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT |
            // VK_PIPELINE_STAGE_LATE_FRAGMENT_TESTS_BIT,
            //	VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_READ_BIT,
...
void vk::ForwardPass::CreateFramebuffer() {
    ...
    VK_CHECK(
        ...
        "Failed to create Forward pass framebuffer.");
}
...
void vk::ForwardPass::BuildDescriptors() {
    ...
    std::vector<VkDescriptorSetLayoutBinding> bindings = {
        CreateDescriptorBinding(
            0, 1, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,
            VK_SHADER_STAGE_VERTEX_BIT |
                VK_SHADER_STAGE_FRAGMENT_BIT), // SceneUBO (projection, view
                                               // etc..)
        CreateDescriptorBinding(1, 1, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,
                                VK_SHADER_STAGE_FRAGMENT_BIT), // Light UBO
        CreateDescriptorBinding(2, 1, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,
                                VK_SHADER_STAGE_FRAGMENT_BIT)};

    meshDescriptorSetLayout = CreateDescriptorSetLayout(context, bindings);

    AllocateDescriptorSets(context, context.descriptorPool,
                           meshDescriptorSetLayout, MAX_FRAMES_IN_FLIGHT,
                           m_descriptorSets);
    ...
    for (size_t i = 0; i < (size_t)MAX_FRAMES_IN_FLIGHT; i++) {
        ...
        UpdateDescriptorSet(context, 0, bufferInfo, m_descriptorSets[i],
    ...
    for (size_t i = 0; i < (size_t)MAX_FRAMES_IN_FLIGHT; i++) {
        ...
        UpdateDescriptorSet(context, 1, bufferInfo, m_descriptorSets[i],
                            VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER);
    }
