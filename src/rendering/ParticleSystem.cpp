...
ParticleSystem::ParticleSystem(Context &aContext, Scene *scene,
                               ParticleSystemSettings const &aSettings)
    : mSettings(aSettings), mContext(aContext), mScene(scene) {
...
void ParticleSystem::DrawAll(VkCommandBuffer cmd, VkPipeline aPipeline, VkPipelineLayout aLayout,
                             const std::array<std::vector<VkDescriptorSet>,
                                              GlobalConfig::maxPlayers> &playerDescriptorSets) {
    for (ParticleSystem* sys : systems)
    {
        sys->Render(cmd, aPipeline, aLayout, playerDescriptorSets);
    }
}
...
void ParticleSystem::Render(VkCommandBuffer cmd, VkPipeline aPipeline, VkPipelineLayout aLayout,
    ...
    for (size_t playerId = 0; playerId < playerCount; ++playerId) {
        ...
        VkViewport viewport = CalcViewport(mContext.extent, playerCount, playerId);
        ...
        vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, aLayout, 0, 1,
                                &playerDescriptorSets[playerId][vkutil::currentFrame], 0, nullptr);

        // bind the particle descriptor
        vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, aLayout, 2, 1,
                                &mDescriptorSet, 0, nullptr);
        // bind the material
        vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, aLayout, 1, 1,
                                &mSettings.material->descriptorSet, 0, nullptr);

        // bind the vertex buffers - positions, texcoords, normals
        VkBuffer buffers[] = {
            mSettings.renderSettings.mesh->meshPrimitives[0].meshGPU->mVertices.buffer};
        VkDeviceSize offsets[] = {0};
        vkCmdBindVertexBuffers(cmd, 0, sizeof(buffers) / sizeof(buffers[0]), buffers, offsets);

        // bind the index buffer
        vkCmdBindIndexBuffer(
            cmd, mSettings.renderSettings.mesh->meshPrimitives[0].meshGPU->mIndices.buffer, 0,
            VK_INDEX_TYPE_UINT32);

        // draw the mesh
        vkCmdDrawIndexed(cmd, mSettings.renderSettings.mesh->meshPrimitives[0].meshGPU->mIndexCount,
                         mSettings.maxParticles, 0, 0, 0);
    }
}
