...
namespace vk {
void TextureManager::addTexture(std::filesystem::path aTexturePath, const std::string& aTextureName) {
    ...
    // load the texture
    Image textureImage = vk::LoadTextureFromDisk(aTexturePath, mContext, VK_FORMAT_R8G8B8A8_UNORM);  // create the texture, issue with SRGB is here, albedo is UNORM
...
TextureManager::TextureManager(Context &aContext) : mContext(aContext) {
