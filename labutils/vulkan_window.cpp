...
namespace
{
    ...
    void create_swapchain_image_views(VkDevice, VkFormat, std::vector < VkImage >
...
namespace labutils
{
    ...
    VulkanWindow::VulkanWindow(VulkanWindow &&aOther)
    ...
    VulkanWindow &VulkanWindow::operator=(VulkanWindow &&aOther)

    noexcept
{
    VulkanContext::operator=(std::move(aOther));

    std::swap( window, aOther
    .window );
    std::swap( surface, aOther
    .surface );
    std::swap( presentFamilyIndex, aOther
    .presentFamilyIndex );
    std::swap( presentQueue, aOther
    .presentQueue );
    std::swap( swapchain, aOther
    .swapchain );
    std::swap( swapImages, aOther
    .swapImages );
    std::swap( swapViews, aOther
    .swapViews );
    std::swap( swapchainFormat, aOther
    .swapchainFormat );
    std::swap( swapchainExtent, aOther
    .swapchainExtent );
    return *this;
}
...
#		endif // ~ debug builds
    ...
    ret.window = glfwCreateWindow(1280, 720, "Vulkan time", nullptr, nullptr);
    if (!ret.window)
    {
        char const *errMsg = nullptr;
        glfwGetError(&errMsg);
    ...
    // Create swap chain
    std::tie(ret.swapchain, ret.swapchainFormat, ret.swapchainExtent) = create_swapchain(ret.physicalDevice,
                                                                                         ret.surface, ret.device,
                                                                                         ret.window,
                                                                                         queueFamilyIndices);
...
SwapChanges recreate_swapchain(VulkanWindow &aWindow)
{
    ...
    // dretermine which properties have changed and return this information
    SwapChanges ret{};
...
namespace
{
    std::vector <VkSurfaceFormatKHR> get_surface_formats(VkPhysicalDevice aPhysicalDev, VkSurfaceKHR aSurface)
    {
        ...
        if (auto const res = vkGetPhysicalDeviceSurfaceFormatsKHR(aPhysicalDev, aSurface, &numFormats, nullptr);
                VK_SUCCESS != res)
        {
            throw lut::Error(
                    "Unable to get surface format count\n vkGetPhysicalDeviceSurfaceFormatsKHR() returned %s\n",
                    lut::to_string(res).c_str());
        }
        std::vector <VkSurfaceFormatKHR> surfaceFormats(numFormats);

        if (auto const res = vkGetPhysicalDeviceSurfaceFormatsKHR(aPhysicalDev, aSurface, &numFormats,
                                                                  surfaceFormats.data()); VK_SUCCESS != res)
        {
    ...
    std::unordered_set <VkPresentModeKHR> get_present_modes(VkPhysicalDevice aPhysicalDev, VkSurfaceKHR aSurface)
    {
        ...
        if (auto const res = vkGetPhysicalDeviceSurfacePresentModesKHR(aPhysicalDev, aSurface, &numPresentModes,
                                                                       nullptr); VK_SUCCESS != res)
        {
            throw lut::Error(
                    "Unable to get surface format count\n vkGetPhysicalDeviceSurfaceFormatsKHR() returned %s\n",
                    lut::to_string(res).c_str());
        ...
        if (auto const res = vkGetPhysicalDeviceSurfacePresentModesKHR(aPhysicalDev, aSurface, &numPresentModes,
                                                                       presentModes.data()); VK_SUCCESS != res)
        {
            throw lut::Error(
                    "Unable to get surface format count\n vkGetPhysicalDeviceSurfaceFormatsKHR() returned %s\n",
                    lut::to_string(res).c_str());
        ...
        // place into set
        std::unordered_set <VkPresentModeKHR> returnerSet;
        for (auto presentMode: presentModes)
        {
            returnerSet.insert(presentMode);
        }
        return returnerSet;
    }

    std::tuple <VkSwapchainKHR, VkFormat, VkExtent2D>
    create_swapchain(VkPhysicalDevice aPhysicalDev, VkSurfaceKHR aSurface, VkDevice aDevice, GLFWwindow *aWindow,
                     std::vector <std::uint32_t> const &aQueueFamilyIndices, VkSwapchainKHR aOldSwapchain)
    {
        ...
        // if there is an 8 bit srgb format pick that. if neither rhna or bgra are available, pick the first one he driver gives us.
        assert(!formats.empty());
        // default to 0
        VkSurfaceFormatKHR format = formats[0];
        for (auto const fmt: formats)
        {
            // if its rgba srgb pick this and quit
            if (VK_FORMAT_R8G8B8A8_SRGB == fmt.format && VK_COLOR_SPACE_SRGB_NONLINEAR_KHR == fmt.colorSpace)
            {
                format = fmt;
                break;
            }
            // if its bgra srgb pick this and quit
        ...
        // FIFO is guaranteed to be available, so is default
        VkPresentModeKHR presentMode = VK_PRESENT_MODE_FIFO_KHR;

        // prefer FIFO releaxed if its an option
        ...
        if (auto const res = vkGetPhysicalDeviceSurfaceCapabilitiesKHR(aPhysicalDev, aSurface, &caps); VK_SUCCESS !=
                                                                                                       res)
        {
            throw lut::Error(
        ...
        // vulkan recommends using minImageCount + 1 - so we use this
        if (imageCount < caps.minImageCount + 1)
        {
            imageCount = caps.minImageCount + 1;
        }
        // make sure that isn't over the max number
        ...
        // set the extent to the size of the frame buffer (clamped by the capabilities)
        ...
        } else
        {
            ...
            // sharing mode concurrent alllows access from multiple queues without transferring ownership.
            // exclusive would require explicit ownership transfers, which is annoying but may result in better performance
    ...
    void get_swapchain_images(VkDevice aDevice, VkSwapchainKHR aSwapchain, std::vector <VkImage> &aImages)
    {
        ...
        if (auto const res = vkGetSwapchainImagesKHR(aDevice, aSwapchain, &numSwapchainImages, nullptr); VK_SUCCESS !=
                                                                                                         res)
        {
            throw lut::Error("Unable to get swapchain image count\n vkGetSwapchainImagesKHR() returned %s",
                             lut::to_string(res).c_str());
        }
        aImages.resize(numSwapchainImages);
        if (auto const res = vkGetSwapchainImagesKHR(aDevice, aSwapchain, &numSwapchainImages, aImages.data());
                VK_SUCCESS != res)
        {
    ...
    void create_swapchain_image_views(VkDevice aDevice, VkFormat aSwapchainFormat, std::vector <VkImage> const &aImages,
                                      std::vector <VkImageView> &aViews)
    {
        ...
        for (std::size_t i = 0; i < aImages.size(); i++)
        {
            ...
            viewInfo.components = VkComponentMapping{VK_COMPONENT_SWIZZLE_IDENTITY, VK_COMPONENT_SWIZZLE_IDENTITY,
...
namespace
{
    ...
    find_queue_family(VkPhysicalDevice aPhysicalDev, VkQueueFlags aQueueFlags, VkSurfaceKHR aSurface)
    {
    ...
    VkDevice create_device(VkPhysicalDevice aPhysicalDev, std::vector <std::uint32_t> const &aQueues,
                           std::vector<char const *> const &aEnabledExtensions)
    {
        ...
        deviceFeatures2.pNext = &imagelessFeatures;
